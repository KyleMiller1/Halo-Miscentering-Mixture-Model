...
def rho_mis_given_r_mis(theta, r, r_mis, phi_samples=100):
    ...
    def sub_integrand(phi, r, r_mis):
        return 1/2 * np.sin(phi) * rho_D22(theta[0:5], np.sqrt(r**2 + r_mis**2 + 2*r*r_mis*np.cos(phi)))
...
def fit_mixture_model(rvals, rhovals, covmats, base_path, out_dir=None, rmis_samples=60, phi_samples=100):
    ...
    def ln_like3d(theta, rvals, rhovals, covmats):
        ...
        r_data = rvals[0]            
        rho_thr_D22 = rho_D22(theta, r_data) 
        ...
        ##########################################
        def integrand(r_mis):                                       
            diff_from_misc = rhovals - rho_mis_given_r_mis(theta, r_data, r_mis, phi_samples=phi_samples)
